...
```{r}
df = read.csv("/Users/oliviawen/singlecell_spca/data/T47D_PHATE.csv")
```
...
phate_plot = subset(big, select=c(PHATE_1, PHATE_2, PHATE_3))
sce_big <- SingleCellExperiment(t(raw_df_big))
colData(sce_big)$phase = big$phase

reducedDims(sce_big) <- list(PHATE=as.matrix(phate_plot))
...
trj <- function(data, sce_want) {
      ...
      #phate coordinates - we use three dimensions
      phate = subset(data, select=c(PHATE_1, PHATE_2, PHATE_3))
      #create SCE object
...
curve_untrt = slingCurves(slo_untrt, as.df = TRUE) %>%
      dplyr::rename("X" = "PHATE_1", "Y" = "PHATE_2", "Z" = "PHATE_3")
print(curve_untrt)
...
write.csv(slingshot_xyz, "/Users/oliviawen/singlecell_spca/data/T47D_PHATE_Curves.csv", row.names=FALSE)
...
write.csv(pseudotimes_all, "/Users/oliviawen/singlecell_spca/data/T47D_PHATE_Pseudotimes.csv", row.names=FALSE)
```