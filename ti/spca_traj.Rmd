...
```{r}
df = read.csv("/Users/oliviawen/singlecell_spca/data/T47D_SPCA.csv")
```

```{r}
#first we make a SCE object of all the data in order to plot the curves atop the SPCA
big = subset(df)
...
spca_plot = subset(big, select=c(SPCA_1, SPCA_2, SPCA_3))
sce_big <- SingleCellExperiment(t(raw_df_big))
colData(sce_big)$phase = big$phase

reducedDims(sce_big) <- list(SPCA=as.matrix(spca_plot))
...
trj <- function(data, sce_want) {
      ...
      #spca coordinates - we use three dimensions
      spca = subset(data, select=c(SPCA_1, SPCA_2, SPCA_3))
      #create SCE object
      sce <- SingleCellExperiment(assays = as.matrix(t(raw_df)))
      colData(sce)$phase = data$phase
      
      reducedDims(sce) <- list(SPCA=as.matrix(spca))
...
curve_untrt = slingCurves(slo_untrt, as.df = TRUE) %>%
      dplyr::rename("X" = "SPCA_1", "Y" = "SPCA_2", "Z" = "SPCA_3")
print(curve_untrt)
...
write.csv(slingshot_xyz, "/Users/oliviawen/singlecell_spca/data/T47D_SPCA_Curves.csv", row.names=FALSE)
...
write.csv(pseudotimes_all, "/Users/oliviawen/singlecell_spca/data/T47D_SPCA_Pseudotimes.csv", row.names=FALSE)
```